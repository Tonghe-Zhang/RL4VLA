...
def main():
    ...
    for split_name, split_files, split_indices in splits:
        ...
        for shard_idx in tqdm(range(num_split_shards), desc=f"Sharding episodes ({split_name})"):
            ...
            for ep_idx, ep_file in tqdm(enumerate(shard_files), desc=f"Parsing shard {shard_idx}", total=len(shard_files)):
                episode_id = split_indices[start + ep_idx]
                try:
...
if __name__ == "__main__":
    main()
    """
    python /nvme_data/tonghe/RL4VLA/episode2dataset_sharded.py \
        --data_dir /nvme_data/tonghe/RL4VLA/datasets/mp_collect/PutOnPlateInScene25Main-v3/12800/data \
        --output_dir /nvme_data/tonghe/RL4VLA/datasets/mp_collect/PutOnPlateInScene25Main-v3/12800/shards-split
    """